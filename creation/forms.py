        ...
        if args:
            if 'foss_category' in args[0]:
                if args[0]['foss_category'] and args[0]['foss_category'] != '' and args[0]['foss_category'] != 'None':
                    ...
                    if 'tutorial_name' in args[0]:
                        initial_data = args[0]['tutorial_name']
                    choices = list(TutorialDetail.objects.filter(id__in=TutorialResource.objects.filter(tutorial_detail__foss_id=args[0][
                                   'foss_category'], language=lang_rec, script_status=4).values_list('tutorial_detail_id')).order_by('order').values_list('id', 'tutorial'))
        ...
        if args:
            if 'foss_category' in args[0]:
                if args[0]['foss_category']:
                    ...
                    choices = list(Language.objects.filter(id__in=TutorialResource.objects.filter(tutorial_detail__in=TutorialDetail.objects.filter(foss_id=int(
                        args[0]['foss_category'])).values_list('id'), status=0).values_list('language_id').distinct()).values_list('id', 'name').order_by('name'))
                    if len(choices):
                    ...
                    if initial_data:
                        ...
                        td_list = TutorialDetail.objects.filter(foss_id=args[0]['foss_category']).values_list('id')
                        choices = list(TutorialResource.objects.filter(tutorial_detail_id__in=td_list, language_id=initial_data, status=0).distinct(
                        ).values_list('tutorial_detail_id', 'tutorial_detail__tutorial'))
                        if len(choices):
                            self.fields['tutorial_name'].widget.attrs = {}
                        choices.insert(0, ('', 'Select Tutorial'))
                        self.fields['tutorial_name'].choices = choices
                        ...
                        if lang.name == 'English':
                            choices = [('outline', 'Outline'), ('script', 'Script'), ('slide', 'Slides'), ('video', 'Video'), (
                                'code', 'Codefiles'), ('assignment', 'Assignment'), ('prerequisite', 'Prerequisite'), ('keyword', 'Keywords')]
                        else:
                ...
                if args[0]['foss_category'] and args[0]['foss_category'] != '' and args[0]['foss_category'] != 'None':
                    ...
                    if 'language' in args[0]:
                        if args[0]['language'] and args[0]['language'] != '' and args[0]['language'] != 'None':
                            initial_data = args[0]['language']
                    choices = list(Language.objects.filter(id__in=TutorialResource.objects.filter(tutorial_detail__in=TutorialDetail.objects.filter(
                        foss_id=int(args[0]['foss_category'])).values_list('id'), status=1).values_list('language_id').distinct()).values_list('id', 'name'))
                    if len(choices):
                        self.fields['language'].widget.attrs = {}
                    choices.insert(0, ('', 'Select Language'))
                    self.fields['language'].choices = choices
                    if initial_data:
                        ...
                        if 'tutorial_name' in args[0]:
                            if args[0]['tutorial_name'] and args[0]['tutorial_name'] != '' and args[0]['tutorial_name'] != 'None':
                                tut_init_data = args[0]['tutorial_name']
                        td_list = TutorialDetail.objects.filter(foss_id=args[0]['foss_category']).values_list('id')
                        choices = list(TutorialResource.objects.filter(tutorial_detail_id__in=td_list, language_id=initial_data, status=1).distinct(
                        ).values_list('tutorial_detail_id', 'tutorial_detail__tutorial'))
                        if len(choices):
                            self.fields['tutorial_name'].widget.attrs = {}
                        choices.insert(0, ('', 'Select Tutorial'))
...
class ComponentForm(forms.Form):
    ...
    def clean(self):
        ...
        if component and component_type:
            if component.content_type not in file_types[component_type]:
    ...
    def __init__(self, comptype, *args, **kwargs):
        ...
        if comptype == 'video':
            ...
            self.fields['isarchive'] = forms.ChoiceField(
                choices=[(0, 'Replace old video'), (1, 'Archive old video as Correction'),
                         (2, 'Archive old video as Version')],
                widget=forms.Select(),
                required=False,
            )
...
class ContributorRoleForm(forms.ModelForm):
    user = forms.ModelChoiceField(
        queryset=User.objects.filter(Q(groups__name='Contributor') | Q(
            groups__name='External-Contributor')).order_by('username'),
        help_text="",
...
class TutorialMissingComponentForm(forms.Form):
    component = forms.ChoiceField(
        choices=[('', 'Select Component'), (1, 'Outline'), (2, 'Script'),
                 (3, 'Video'), (4, 'Slides'), (5, 'Codefiles'), (6, 'Assignment')],
        required=True,
    ...
    def __init__(self, *args, **kwargs):
        ...
        if args:
            if 'foss' in args[0] and args[0]['foss']:
                ...
                choices = TutorialResource.objects.filter(Q(status=1) | Q(status=2), tutorial_detail__foss_id=args[0][
                                                          'foss']).values_list('language_id', 'language__name').order_by('language__name').distinct()
